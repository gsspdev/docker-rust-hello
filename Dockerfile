...
FROM --platform=linux/arm64 tonistiigi/xx:1.3.0 AS xx

################################################################################
# Create a stage for building the application.
FROM --platform=linux/arm64 rust:latest AS build
ARG APP_NAME
WORKDIR /app
...
RUN --mount=type=bind,source=src,target=src \
    ...
    --mount=type=cache,target=/app/target/,id=rust-cache-${APP_NAME}-linux/arm64 \
